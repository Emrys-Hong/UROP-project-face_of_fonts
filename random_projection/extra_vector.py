...
class Random_projection:
    ...
    def extra_vectors_X_distance(self, extra_vectors):
        self.extra_vec_X_distance = np.dot(extra_vectors, self.X.T)
    ...
    def control_variant(self, i, j):
        for x in range(len(self.extra_vectors)):
            for y in range(len(self.extra_vectors)):

                alpha = self.extra_vec_X_distance[x][i]*self.extra_vec_X_distance[y][j] + self.extra_vec_X_distance[y][i]*self.extra_vec_X_distance[x][j]
                if x != y:
                    self.V_V_distance[i][j] -= 0.5 * alpha * np.dot(self.extra_vectors[x],self.extra_vectors[y])
                elif x == y:
                    self.V_V_distance[i][j] -= 0.5 * (alpha-1) * np.dot(self.extra_vectors[x], self.extra_vectors[y])
...
if __name__ == '__main__':
    ...
    for i in range(len(V_V_distance)):
        for j in range(len(V_V_distance)):
            projection1.control_variant(i,j)
